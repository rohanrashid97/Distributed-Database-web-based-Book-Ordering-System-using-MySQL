...
$users_no = $conn1->query("SELECT * FROM users_info WHERE user_type='User'") or die('query failed');
$usercount = mysqli_num_rows($users_no);
$admin_no = $conn1->query("SELECT * FROM users_info WHERE user_type='Admin' ") or die('query failed');
$admin_count = mysqli_num_rows($admin_no);
$books_no = $conn1->query("SELECT * FROM book_info ") or die('query failed');
$bookscount = mysqli_num_rows($books_no);
$orders = $conn1->query("SELECT * FROM confirm_order ") or die('query failed');
$orders_count = mysqli_num_rows($orders);
$msg_no = $conn1->query("SELECT * FROM msg ") or die('query failed');
$msgcount = mysqli_num_rows($msg_no);
$date = date('d-m-Y'); // Assuming date format is YYYY-MM-DD
$result = $conn1->query("SELECT sum(total_price) as total FROM confirm_order WHERE date = '$date'") or die('Query failed');
...
<div class="main_box">
    <div class="card" style="width: 15rem">
        <?php
        $total_pendings = 0;
        $select_pending = mysqli_query($conn1, "SELECT total_price FROM `confirm_order` WHERE payment_status = 'pending'") or die('query failed');
        ...
        <div class="card-body">
            ...
            <p class="card-text">
                <span style="font-size: 18px;">Ord: </span>
    ...
    <div class="card" style="width: 15rem">
        <?php
        $total_completed = 0;
        $select_completed = mysqli_query($conn1, "SELECT total_price FROM `confirm_order` WHERE payment_status = 'completed'") or die('query failed');
        ...
        <div class="card-body">
            <h5 class="card-title">Number Of Completed Orders in TK</h5>
            <p class="card-text">
    ...
    <div class="card" style="width: 15rem">
        ...
        <div class="card-body">
            ...
            <a href="admins_detail.php" class="btn btn-primary">Details</a>
        </div>
    </div>
    <div class="card" style="width: 15rem">
    ...
    <div class="card" style="width: 15rem">
        ...
        <div class="card-body">
            ...
            <p class="card-text">
                <?php if ($row && isset($row['total'])) {
                    echo $row['total']; // Output the total
                } else {
                    echo 0;
                }
                ?>
            </p>
            <a href="order_filter.php" class="btn btn-primary">Details</a>
        </div>
    </div>
