...
if(isset($_GET['remove'])){
    ...
    mysqli_query($conn1, "DELETE FROM `cart` WHERE id='$remove_id'") or die('query failed');
...
if(isset($_POST['update'])){
    ...
    mysqli_query($conn1, "UPDATE `cart` SET `quantity`='$update_quantity', `total`='$total_price' WHERE `id`='$update_cart_id'") or die('query failed');
    ...
    ?>
        <table style="width: 70%; align-items:center; margin:10px auto;" >
            ...
            <tbody>
                ...
                <?php
                $total = 0;
                $select_book = $conn1->query("SELECT id, name,price, image ,quantity,total  FROM cart Where user_id= $user_id");
                ...
                <tr>
                    ...
                    <td>
                        <form action="" method="POST">
                            <input type="number" name="update_quantity" min="1" max="3" value="<?php echo $row['quantity'];?>">
        ...
        <a href="checkout.php" class="btn cart-btn1" style="display:<?php if($total>1){ echo 'inline-block'; }else{ echo 'none'; };?>" > &nbsp; Proceed to Checkout</a> 
