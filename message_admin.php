...
if (isset($_GET['delete_msg'])) {
    ...
    mysqli_query($conn1, "DELETE FROM `msg` WHERE id = '$msg_id'") or die('query failed');
    mysqli_query($conn2, "DELETE FROM `msg` WHERE id = '$msg_id'") or die('query failed');
    mysqli_query($conn3, "DELETE FROM `msg` WHERE id = '$msg_id'") or die('query failed');
...
<body>
   ...
   <?php
      $select_user = mysqli_query($conn1, "SELECT id,name,email,number,msg,date FROM msg") or die('query failed');
