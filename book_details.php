...
if (isset($_POST['add_to_cart'])) {
    if(!isset($user_id)){
        ...
        $select_book = $conn1->query("SELECT * FROM cart WHERE name= '$book_name' AND user_id='$user_id' ") or die('query failed');

        if (mysqli_num_rows($select_book) > 0) {
            $message[] = 'This Book is alredy in your cart';
        } else {
            if(isset($_SESSION['user_branch'])){
                $conn1->query("INSERT INTO cart (`book_id`,`user_id`,`name`, `price`, `image`, `quantity` ,`total`) VALUES('$book_id','$user_id','$book_name','$book_price','$book_image','$book_quantity', '$total_price')") or die('Add to cart Query failed');
...
<body>
    ...
    <div class="details">
        ...
        if (isset($_GET['details'])) {
            $get_id = $_GET['details'];
            $get_book = mysqli_query($conn1, "SELECT * FROM `book_info` BI inner join book_stock_view bsv on BI.bid = bsv.bid WHERE BI.bid = '$get_id'") or die('query failed');
                    ...
                    <div class="row_box">
                        <form style="display: flex ;" action="" method="POST">
                            ...
                            <div class="col_box">
                                ...
                                <h3>Price: TK <?php echo $fetch_book['price']; ?>/-</h3>
                                <h3><?php if($fetch_book['available_qty'] > 0){echo "Available: ".$fetch_book['available_qty']."pcs";}else{echo '<td class="text-center" data-status="4"><span class="badge badge-danger">Out of Stock</span></td>';} ?></h3>
                                <label for="quantity">Quantity:</label>
                                <input type="number" name="quantity" value="1" min="1" max="3" id="quantity">
                                <div class="buttons">
                                    ...
                                    <input class="hidden_input" type="hidden" name="book_quantity" >
