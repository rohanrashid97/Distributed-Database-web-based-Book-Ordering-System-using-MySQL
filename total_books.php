...
if(isset($_GET['delete'])){
  ...
  mysqli_query($conn1, "DELETE FROM `book_info` WHERE bid = '$delete_id'") or die('query failed');
  mysqli_query($conn2, "DELETE FROM `book_info` WHERE bid = '$delete_id'") or die('query failed');
  mysqli_query($conn3, "DELETE FROM `book_info` WHERE bid = '$delete_id'") or die('query failed');
...
if(isset($_POST['update_product'])){
  ...
  mysqli_query($conn1, "UPDATE `book_info` SET name = '$update_name', title='$update_title', description ='$update_description', price = '$update_price', category='$update_category' WHERE bid = '$update_p_id'") or die('query failed');
  mysqli_query($conn2, "UPDATE `book_info` SET name = '$update_name', title='$update_title', description ='$update_description', price = '$update_price', category='$update_category' WHERE bid = '$update_p_id'") or die('query failed');
  mysqli_query($conn3, "UPDATE `book_info` SET name = '$update_name', title='$update_title', description ='$update_description', price = '$update_price', category='$update_category' WHERE bid = '$update_p_id'") or die('query failed');
  ...
  if(!empty($update_image)){
     if($update_image_size > 2000000){
        ...
        mysqli_query($conn1, "UPDATE `book_info` SET image = '$update_image' WHERE bid = '$update_p_id'") or die('query failed');
        mysqli_query($conn2, "UPDATE `book_info` SET image = '$update_image' WHERE bid = '$update_p_id'") or die('query failed');
        mysqli_query($conn3, "UPDATE `book_info` SET image = '$update_image' WHERE bid = '$update_p_id'") or die('query failed');
...
<section class="edit-product-form">
  ...
  <section class="show-products">
   <div class="box-container">
      ...
      <?php
         $select_book = mysqli_query($conn1, "SELECT * FROM `book_info` BI inner join book_stock_view bsv on BI.bid = bsv.bid ORDER BY date DESC") or die('query failed');
      ...
      <div class="box">
         ...
         <div style="" class="quantity">In Stock: <?php echo $fetch_book['available_qty'];?>Pcs</div>
