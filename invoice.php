...
if(isset($_GET['order_id'])){
  ...
  $get_order = mysqli_query($conn1, "SELECT * FROM `confirm_order` WHERE order_id = '$order_id'") or die('query failed');
  if (mysqli_num_rows($get_order) > 0){
    $fetch_order = mysqli_fetch_assoc($get_order);
    
  }
  $get_order = mysqli_query($conn1, "SELECT * FROM `orders` WHERE id = '$order_id'") or die('query failed');
  if (mysqli_num_rows($get_order) > 0){
  ...
  <body>
    <div class="invoice">
          <div class="section-top">
            <div class="logo">
                <a><span>Book</span>
                    <span class="me">Ordering System</span></a>
            </div>
        ...
        <tr>
          ...
          <div class="buyer-details">
              <p class="buyer-name">By,  BooksOrderingSystem</p>
              <p class="buyer-add">Mirpur</p>
              <p class="buyer-city">Dhaka</p>
          </div>
      </div></th>
...
">
        <table style="width: 100%;">
          ...
          $select_book = mysqli_query($conn1, "SELECT * FROM `orders`WHERE id = '$order_id'") or die('query failed');
          $s=1;
      ...
      <hr />
      <div>
        <div class="sign">BooksOrderingSystem</div>
      </div>
    </div>
  </body>
</html>';
